 ...
 */
export default function BelegSuche({ filter, onChange, kategorien = [], anzahl, gesamt, onExport, isMobile }) {
    ...
    const suchfeld = (
        <div style={{ position: "relative", flex: "1 1 170px", minWidth: 130, maxWidth: isMobile ? "none" : 260 }}>
            ...
            <input
                ...
                style={{ padding: "6px 26px 6px 28px", border: "1px solid #e5e7eb", borderRadius: 7, fontSize: 12, color: "#374151", background: "#f9fafb", width: "100%", outline: "none", fontFamily: "inherit" }}
            />
            {filter.query && (
    ...
    const filterKnopf = (
        <button onClick={() => setOffen((v) => !v)}
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 11px", border: `1px solid ${offen || anzahlFilter ? "#18537a" : "#e5e7eb"}`, background: offen || anzahlFilter ? "#eef4f8" : "#f9fafb", color: offen || anzahlFilter ? "#18537a" : "#6b7280", borderRadius: 7, fontSize: 12, fontWeight: 500, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0, fontFamily: "inherit" }}>
    ...
    const exportKnopf = onExport && (
        <button onClick={() => onExport(filter)} disabled={!anzahl}
                ...
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 11px", border: "1px solid #e5e7eb", background: anzahl ? "#f9fafb" : "#f3f4f6", color: anzahl ? "#6b7280" : "#d1d5db", borderRadius: 7, fontSize: 12, fontWeight: 500, cursor: anzahl ? "pointer" : "default", fontFamily: "inherit", whiteSpace: "nowrap", flexShrink: 0 }}>
    ...
    return (
        <>
            {/* Kopfzeile. Am Telefon zwei Zeilen: oben Überschrift und Export,
                darunter Suche und Filter – nebeneinander wird es zu eng. */}
            {isMobile ? (
                <div style={{ padding: "14px 16px", borderBottom: "1px solid #f0ece4", display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: "14px 20px", borderBottom: "1px solid #f0ece4", flexWrap: "wrap" }}>
                    {ueberschrift}
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", justifyContent: "flex-end", flex: "1 1 auto", minWidth: 0 }}>
                        {suchfeld}
                        {filterKnopf}
                        {exportKnopf}
                    </div>
